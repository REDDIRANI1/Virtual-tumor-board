...
- Q: What concurrency mechanism guards against conflicting Moderator state transitions? → A: Optimistic locking — a `version` integer field on `Case`; every state-transition write increments the version and rejects the request with HTTP 409 if the submitted version does not match the current DB value.
- Q: How are Doctors notified when invited to a case? → A: In-app only — an `Invitation` record is persisted in the database; no email backend required.
- Q: What is the real-time implementation scope? → A: Design document only — a written Django Channels + Redis architecture section covering group naming, auth checks, event payload shape, and bottleneck analysis for 50 concurrent Doctors; no code produced.
- Q: How are audit events stored and created? → A: Dedicated `AuditEvent` model — service methods explicitly create an `AuditEvent` record after each clinically meaningful write; no Django signals; supports a Moderator-accessible `/audit/` endpoint and direct assertion in tests.
...
- **FR-005**: Moderators MUST be able to invite Doctors to a case discussion panel. Invitation is recorded as an in-app `Invitation` record only; no email notification is required. The Doctor discovers the invitation via their case list or a dedicated invitations endpoint.
...
- **Audit Event**: A dedicated `AuditEvent` model record created explicitly by service methods (not Django signals) after each clinically meaningful write: state transitions, invitations, identity reveals, answer publication, and answer amendments. Supports a Moderator-accessible read-only `/audit/` endpoint. Ordinary reads do not create audit records.
...
- **Concurrency**: Optimistic locking via a `version` integer field on the `Case` model. Every state-transition request must submit the current version; the write atomically increments it and raises HTTP 409 (Conflict) if the submitted version is stale. Cost: concurrent read-then-write pairs may produce more client-visible 409 retries under high contention than `select_for_update`, but avoids long-held row locks and scales better under read-heavy workloads. Rejected alternatives: `select_for_update` (holds lock for full transaction duration, risk of lock contention under load); advisory locks (complex lifecycle management, harder to test).
...
- **Auditability without bloat**: Dedicated `AuditEvent` model; service methods explicitly create records for: state transition, Doctor invitation, identity reveal, answer publish, answer amendment. Django signals are explicitly rejected (too implicit, harder to test, risk of double-firing). Ordinary reads, list views, and detail views do not create audit records. The line is drawn at writes that change clinical state or identity presentation.
...
- **Real-time design**: Design document only. The design document MUST include a Django Channels + Redis architecture section covering: WebSocket group naming scheme per case, JWT-based auth handshake for WS connections, event payload shape for new comments and status updates, and a specific bottleneck analysis for 50 Doctors active on one case simultaneously. No implementation code is produced for this submission.
...
- Real-time comment updates are covered in the design document only (Django Channels + Redis architecture); no real-time code is in scope for this submission.
