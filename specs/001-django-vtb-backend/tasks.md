...
- [ ] T003 [P] Create split settings: `config/settings/__init__.py`, `config/settings/base.py` (INSTALLED_APPS, REST_FRAMEWORK, SIMPLE_JWT with 15-min access / 1-day refresh / blacklisting / rotation, AUTH_USER_MODEL='accounts.User', DEFAULT_AUTO_FIELD='django.db.models.UUIDField'), `config/settings/dev.py` (DEBUG=True, local PG, CORS), `config/settings/test.py` (fast test config), `config/settings/prod.py` (stub)
- [ ] T004 [P] Create `.env.example` with DATABASE_URL, SECRET_KEY, JWT_SIGNING_KEY, DJANGO_SETTINGS_MODULE and configure `config/settings/base.py` to load env vars (use `environ` or manual os.environ)
...
- [ ] T010 [P] Create `apps/accounts/tests/__init__.py` and `apps/accounts/tests/factories.py` with UserFactory (username=Sequence, email=LazyAttribute, role='doctor' default)
...
- [ ] T013 Add model-level immutability safeguards in `apps/cases/models.py`: Comment.save() override raises error if case.status=='ANSWERED' and this is an update to content (not a reveal); PublishedAnswer.save() override rejects updates on existing PKs; AuditEvent.save() rejects updates, AuditEvent.delete() always raises
...
- [ ] T022 [P] Create `apps/cases/tests/__init__.py` and `apps/cases/tests/factories.py` with CaseFactory (warrior=SubFactory(UserFactory, role='warrior'), status='SUBMITTED'), InvitationFactory (case=SubFactory(CaseFactory), doctor=SubFactory(UserFactory, role='doctor')), CommentFactory, PublishedAnswerFactory, AmendedAnswerFactory
...
- [ ] T026 [P] [US1] Write test for Warrior safe-view in `apps/cases/tests/test_visibility.py`: Warrior viewing case detail before publish sees status and original_question but NOT doctor comments, NOT unpublished answer drafts, NOT internal structured_summary details
...
- [ ] T039 [US2] Implement reveal_comment_identity service in `apps/cases/services.py`: validate requesting_user is comment author, validate comment is_anonymous and not is_revealed, set is_anonymous=False and is_revealed=True (save update_fields only), create IDENTITY_REVEALED audit event. Do NOT update any other comments' snapshots
...
- [ ] T049 [P] [US3] Write Warrior visibility test for final answer in `apps/cases/tests/test_visibility.py`: Warrior sees published answer content after ANSWERED; Warrior sees amendment history if implemented; Warrior still cannot see doctor comments after ANSWERED
...
- [ ] T058 [P] [US4] Write audit event tests in `apps/audit/tests/test_audit.py`: audit events created for transition, structure, invite, comment, reveal, publish, amend; ordinary GET requests do NOT create audit events; AuditEvent update/delete raise errors; Moderator can read audit trail; non-Moderator gets 403
...
- [ ] T064 [US4] Run full test suite and verify all pass: `pytest -v --tb=short`
