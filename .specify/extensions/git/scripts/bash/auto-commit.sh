...
# Stage and commit via the repository's required commit wrapper.
# Project policy forbids invoking `git commit` directly.
_commit_script="$REPO_ROOT/commit.sh"
if [ ! -f "$_commit_script" ]; then
    echo "[specify] Error: commit.sh not found; skipped auto-commit" >&2
    exit 1
fi

_commit_out=$(bash "$_commit_script" "$_commit_msg" 2>&1) || {
    echo "[specify] Error: commit.sh failed: $_commit_out" >&2
    exit 1
}

echo "[OK] Changes committed ${_phase} ${_command_name} using commit.sh" >&2
